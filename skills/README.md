...
- **council** — adversarial two-model debate: Claude and GPT-5.5 generate independent positions, challenge each other for up to 3 rounds, converge on the strongest answer by argument quality — not agreement. Anti-sycophancy guards throughout.
...
- **multi-review** — multi-angle code review. Fans out up to eight reviewers in parallel (four Claude subagents: broad generalist + correctness/numerics, GPU performance, code quality lenses; plus four GPT-5.5 `codex exec` calls, one per active lens), consolidates and validates findings against source, dedups against existing PR reviews in PR mode, and reports a merged severity-ranked review. No GitHub posting. `REFERENCE.md` holds the per-lens checklists; `gather_context.sh` builds the diff/chunk context.
