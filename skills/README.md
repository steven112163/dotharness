...
- **multi-review** — multi-angle code review. Fans out eight reviewers in parallel (four Claude subagents: broad generalist + correctness/numerics, GPU performance, code quality lenses; plus three `codex exec` background calls for GPT/Gemini and one `bin/llm` call for DeepSeek), consolidates and validates findings against source, dedups against existing PR reviews in PR mode, and reports a merged severity-ranked review. No GitHub posting. `REFERENCE.md` holds the per-lens checklists; `gather_context.sh` builds the diff/chunk context.
