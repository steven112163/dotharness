...
if [ -z "$RPC" ]; then
  ...
  dxroot "apt-get install -y rocprofiler-compute || (apt-get update && apt-get install -y rocprofiler-compute)" \
    || { echo 'ERROR: rocprofiler-compute install failed; reporting rather than forcing.' >&2; exit 2; }
  RPC=$(dx "$_resolve_rpc")
fi
[ -z "$RPC" ] && { echo 'ERROR: rocprof-compute not found after install.' >&2; exit 2; }
...
VENV="${VENV:-$HOME/pyenv/rocprof-compute-py$PYVER}"; PYBIN="$VENV/bin/python"
if ! dx "test -x '$PYBIN' && '$PYBIN' '$RPC_REAL' --version >/dev/null 2>&1"; then
  [ -z "$REQ" ] && { echo 'ERROR: requirements.txt not found for rocprof-compute.' >&2; exit 2; }
  ...
  if dx "command -v uv >/dev/null 2>&1"; then
    ...
    dx "uv venv --system-site-packages --python python3 '$VENV' && uv pip install -q --python '$PYBIN' -r '$REQ'" \
      || { echo 'ERROR: uv failed to build rocprof-compute venv.' >&2; exit 2; }
  else
    dx "python3 -m venv --system-site-packages '$VENV' && '$PYBIN' -m pip install -q --upgrade pip && '$PYBIN' -m pip install -q -r '$REQ'" \
      || { echo 'ERROR: failed to install rocprof-compute python deps into venv.' >&2; exit 2; }
  fi
fi
...
csvname="${WORKLOAD}_csv"; csvdir="$RAW/$csvname"
