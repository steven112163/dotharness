...
`setup.sh` always re-applies `third-party/geak`'s sparse-checkout (`perf_knowledge/` plus GEAK's repository-root files, since that state doesn't travel via `.gitmodules`); symlinks `bin/` scripts into `~/bin/`; symlinks `lib/` subdirs into `~/lib/`; symlinks `gitignore_global` to `~/.gitignore_global` and registers it as `git config --global core.excludesFile`; provisions a repo-local `.venv` with `pre-commit` and `anthropic`; and installs `playwright-cli` (npm) and `graphify` (pipx) globally. If the `claude` CLI is present, it also symlinks `skills/`, `agents/`, `hooks/`, `rules/`, `output-styles/`, third-party skills, and `statusline.sh` into `~/.claude/`; registers lifecycle hooks in `~/.claude/settings.json`; and installs Claude plugins. If the `codex` CLI is present, it also mirrors skills/hooks/statusline into `~/.agents/` and `~/.codex/`, concatenates `rules/*.md` into `~/.agents/AGENTS.md`, and runs `graphify install --platform codex`. Re-running is idempotent. Requires `jq`.
...
- `multi-review` — up to 8 parallel reviewers (Claude `reviewer` subagents + GPT-6-astra codex calls per lens); one merge-and-validate pass; dedup against existing PR reviews. Modes: PR number or URL, `repo` (whole tree), or no argument (branch vs merge-base, working tree included).
...
- `template/` also holds copyable config: `ckremote.example` (`~/.config/ckremote`, server list),
  `ckdockersetup.example` (`~/.config/ckdockersetup`, `DOCKER_SETUP_CMD` for `ckCommon`'s
  one-time docker-commit setup step), and `ckdevcontainer.example` (`~/.config/ckdevcontainer`,
  `DEV_CONTAINER_NAME` — a persistent container from `bin/dockerRun` that `ckCommon`'s docker
  backend reuses via `docker exec` instead of a fresh `--rm` container, when running).
