...
- Build and self-check before declaring done. For CK targets build with `ckBuild`:
  `REPO=<worktree> ckBuild configure gfx942` (only needed once, or after an
  arch/cmake-option change) then `REPO=<worktree> ckBuild build <target>`; do not
  hand-roll cmake/ninja. Run the built binary on a GPU for a quick self-check with
  `ckRun` (`REPO=<worktree> ckRun --arch <gfx> <cmd>`).
